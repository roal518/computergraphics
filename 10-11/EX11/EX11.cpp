...
#include <gl/glm/glm/glm.hpp>//파일위치 확인하고 바꾸자 지금은 데스크탑 위치임
...
void draw3Q();
void draw4Q();
float returnintiger(int setmode, float coordinate);
...
void keyboard(unsigned char key, int x, int y) {
	if (key == 'i') {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 3; j++) {
				rect1st[i][j].x = returnintiger(1, rect1st[i][j].x);
				rect1st[i][j].y = returnintiger(1, rect1st[i][j].y);
			}
		}
		normalmode = false;
		mode[0] = true;
		...
		makeshape();
		for (int j = 0; j < 3; j++) {
			trian2nd[j].x = returnintiger(1, trian2nd[j].x);
			trian2nd[j].y = returnintiger(1, trian2nd[j].y);
		}
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				rect3rd[i][j].x = returnintiger(1, rect3rd[i][j].x);
				rect3rd[i][j].y = returnintiger(1, rect3rd[i][j].y);
			}
		}
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				penta4th[i][j].x = returnintiger(1, penta4th[i][j].x);
				penta4th[i][j].y = returnintiger(1, penta4th[i][j].y);
			}
		}
		normalmode = false;
		mode[0] = false;
...
void secondquad(int val) {
	if (mode[1]) {
		if (normalmode) {
			if (trian2nd[2].y <= returnintiger(2, 0.7)) {
				trian2nd[2].y += 0.001;
			}
			if (trian2nd[2].x >= returnintiger(2, -0.5)) {
				trian2nd[2].x -= 0.001;
			}
		}
		else {
			if (trian2nd[2].y <= returnintiger(1, 0.7)) {
				trian2nd[2].y += 0.001;
			}
			if (trian2nd[2].x >= returnintiger(1, -0.5)) {
				trian2nd[2].x -= 0.001;
			}
		}
		glutPostRedisplay();
...
void thirdquad(int val) {
	if (mode[2]) {
		if (normalmode) {
			if (rect3rd[0][0].x < returnintiger(2, -0.7)) {
				rect3rd[0][0].x += 0.001;
			}
			if (rect3rd[0][0].y > returnintiger(2, -0.8)) {
				rect3rd[0][0].y -= 0.001;
			}
			if (rect3rd[0][1].y > returnintiger(2, -0.8)) {
				rect3rd[0][1].y -= 0.001;
				rect3rd[1][1].y -= 0.001;
				rect3rd[2][1].y -= 0.001;
			}
			if (rect3rd[0][2].y > returnintiger(2, -0.4)) {
				rect3rd[0][2].y -= 0.001;
				rect3rd[1][0].y -= 0.001;
			}
			if (rect3rd[0][2].x > returnintiger(2, -0.3)) {
				rect3rd[1][0].x -= 0.001;
				rect3rd[2][1].x -= 0.001;
			}
			if (rect3rd[1][2].x > returnintiger(2, -0.45)) {
				rect3rd[1][2].x -= 0.001;
				rect3rd[2][0].x -= 0.001;
			}
			if (rect3rd[2][2].x < returnintiger(2, -0.2)) {
				rect3rd[2][2].x += 0.001;
			}
			if (rect3rd[2][2].y > returnintiger(2, -0.4)) {
				rect3rd[2][2].y -= 0.001;
			}

		}
		else {
			if (rect3rd[0][0].x < returnintiger(1, -0.7)) {
				rect3rd[0][0].x += 0.001;
			}
			if (rect3rd[0][0].y > returnintiger(1, -0.8)) {
				rect3rd[0][0].y -= 0.001;
			}
			if (rect3rd[0][1].y > returnintiger(1, -0.8)) {
				rect3rd[0][1].y -= 0.001;
				rect3rd[1][1].y -= 0.001;
				rect3rd[2][1].y -= 0.001;
			}
			if (rect3rd[0][2].y > returnintiger(1, -0.4)) {
				rect3rd[0][2].y -= 0.001;
				rect3rd[1][0].y -= 0.001;
			}
			if (rect3rd[0][2].x > returnintiger(1, -0.3)) {
				rect3rd[1][0].x -= 0.001;
				rect3rd[2][1].x -= 0.001;
			}
			if (rect3rd[1][2].x > returnintiger(1, -0.45)) {
				rect3rd[1][2].x -= 0.001;
				rect3rd[2][0].x -= 0.001;
			}
			if (rect3rd[2][2].x < returnintiger(1, -0.2)) {
				rect3rd[2][2].x += 0.001;
			}
			if (rect3rd[2][2].y > returnintiger(1, -0.4)) {
				rect3rd[2][2].y -= 0.001;
			}
		}
		glutPostRedisplay();
	...
	if (mode[3]) {
		if (mode[3]) {
			if (normalmode) {
				...
				if (penta4th[0][0].x < returnintiger(2, 0.68f)) {
					penta4th[0][0].x += 0.001;
					penta4th[1][0].x += 0.001;
				}
				if (penta4th[0][0].y > returnintiger(2, -0.79f)) {
					penta4th[0][0].y -= 0.001;
					penta4th[1][0].y -= 0.001;
				}
				if (penta4th[0][1].x < returnintiger(2, 0.685f)) {
					penta4th[0][1].x += 0.001;
				}
				if (penta4th[1][2].x < returnintiger(2, 0.69)) {
					penta4th[2][0].x += 0.001;
					penta4th[1][2].x += 0.001;
				}
				if (penta4th[1][2].y > returnintiger(2, -0.78)) {
					penta4th[1][2].y -= 0.001;
					penta4th[2][0].y -= 0.001;
				}
				if (penta4th[2][2].x > returnintiger(2, 0.703)) {
					penta4th[2][2].x -= 0.001;
				}
				if (penta4th[2][2].y > returnintiger(2, -0.79)) {
					penta4th[2][2].y -= 0.001;
				}
			}
			else {
				if (penta4th[0][0].x < returnintiger(1, 0.68f)) {
					penta4th[0][0].x += 0.001;
					penta4th[1][0].x += 0.001;
				}
				if (penta4th[0][0].y > returnintiger(1, -0.79f)) {
					penta4th[0][0].y -= 0.001;
					penta4th[1][0].y -= 0.001;
				}
				if (penta4th[0][1].x < returnintiger(1, 0.685f)) {
					penta4th[0][1].x += 0.001;
				}
				if (penta4th[1][2].x < returnintiger(1, 0.69)) {
					penta4th[2][0].x += 0.001;
					penta4th[1][2].x += 0.001;
				}
				if (penta4th[1][2].y > returnintiger(1, -0.78)) {
					penta4th[1][2].y -= 0.001;
					penta4th[2][0].y -= 0.001;
				}
				if (penta4th[2][2].x > returnintiger(1, 0.703)) {
					penta4th[2][2].x -= 0.001;
				}
				if (penta4th[2][2].y > returnintiger(1, -0.79)) {
					penta4th[2][2].y -= 0.001;
				}
			}
			glutPostRedisplay();

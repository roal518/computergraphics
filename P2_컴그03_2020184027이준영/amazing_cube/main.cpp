...
void keyboard(unsigned char key, int x, int y) {
	...
	if (key == '4') {
		...
		for (int i = 0; i < info_x; i++) {
			...
			if (info_x > info_z) {
				for (int i = 0; i < info_x; i++) {
					for (int j = 0; j < info_z; j++) {
						inform[i][j].increaser = 0.001f * ((i + j) % info_x);
					}
				}
			}
			else {
				for (int i = 0; i < info_z; i++) {
					for (int j = 0; j < info_x; j++) {
						inform[j][i].increaser = 0.001f * ((i + j) % info_z);
					}
				}
			}
		}
	...
	if (key == '5') {
		...
		for (int i = 0; i < info_x; i++) {
			for (int j = 0; j < info_z; j++) {
				inform[i][j].height = 0.0f;
				inform[i][j].increaser = 0.1 * static_cast<float>(rand()) / RAND_MAX;
				inform[i][j].touch_ceil = false;
			}
		}
		if (info_x > info_z) {
			for (int i = 0; i < info_x; i++) {
				for (int j = 0; j < info_z - 1; j++) {
					for (int k = 0; k < info_z - j - 1; k++) {
						if (inform[i][k].increaser > inform[i][k + 1].increaser) {
							std::swap(inform[i][k], inform[i][k + 1]);
						}
					}
				}
			}

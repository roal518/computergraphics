...
float y_crane_move = 0.0f;
float z_crane_move = 0.0f;
...
void keyboard(unsigned char key, int x, int y) {
	...
	if (key == 'w') {
		...
		x_crane_move -= 10 * glm::cos(glm::radians(camYaw));
		z_crane_move -= 10 * glm::sin(glm::radians(camYaw));
	}
	if (key == 's') {
		x_crane_move += 10 * glm::cos(glm::radians(camYaw));
		z_crane_move += 10 * glm::sin(glm::radians(camYaw));
	...
	if (key == 'a') {
		x_crane_move -= 10 * glm::sin(glm::radians(camYaw));
		z_crane_move += 10 * glm::cos(glm::radians(camYaw));
	}
	if (key == 'd') {
		x_crane_move += 10 * glm::sin(glm::radians(camYaw));
		z_crane_move -= 10 * glm::cos(glm::radians(camYaw));
...
void draw_head() {
	...
	modelMatrix = glm::translate(modelMatrix, glm::vec3(x_crane_move, y_crane_move, z_crane_move));
	modelMatrix = glm::rotate(modelMatrix, glm::radians(-camYaw), glm::vec3(0.0f, 1.0f, 0.0f));
	modelMatrix = glm::rotate(modelMatrix, glm::radians(y_crane_theta_mom), glm::vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(modelMatrixLocation, 1, GL_FALSE, glm::value_ptr(modelMatrix));
...
void viewproj() {
	...
	glm::vec3 camPos = glm::vec3(50  + x_crane_move, y_crane_move + 20, 50+z_crane_move);
	glm::vec3 camTag = glm::vec3(x_crane_move, y_crane_move, z_crane_move);

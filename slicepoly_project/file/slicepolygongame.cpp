...
void Mouseclick(int button, int state, int x, int y) {
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		...
		printf("X:%f\tY:%f\n", horse->vertex[1].x, horse->vertex[1].y);
		horse->vertex[0].x =0;
		horse->vertex[0].y =0;
		horse->vertex[1].x =0;
		horse->vertex[1].y =0;
...
void Delete(int val) {
	...
	while (current != NULL) {
		if (current->theta > 180.f && current->vertex_counter>2) {
			if (previous == NULL) {
...
void draw_poly(Node* list) {
	Node* horse = list;
	while (horse->next != NULL) {//리스트를 순회하면서 polygon draw
